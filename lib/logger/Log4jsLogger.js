...
var Log4jsLogger = function (_Logger) {
  ...
  (0, _createClass3.default)(Log4jsLogger, null, [{
    ...
    value: function getCoreLogger(options) {
      var logger = Log4jsLogger.CORE.getLogger(options.category);
    ...
    value: function logMiddleware() {
      ...
      var logger = new Log4jsLogger({ category: options.category || 'request' });
  ...
  function Log4jsLogger() {
    var options = arguments.length <= 0 || arguments[0] === undefined ? { category: 'app', level: 'info', replaceConsole: false } : arguments[0];
